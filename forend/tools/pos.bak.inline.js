...
const TAX_RATE = 0.19; // 19% IVA (ajusta según país)
let catalog = [];
let filteredCatalog = [];
let cart = [];
let lastSale = null;
...
function ensureZXing(){
  ...
  return new Promise((resolve,reject)=>{
    ...
    script.onerror=()=>reject(new Error("No se pudo cargar el lector de cÃ³digos."));
...
async function startScanCamera(){
  ...
  try{
    ...
    if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){
      if(status) status.textContent="Tu navegador no permite usar la cámara. Usa el lector de código.";
      return;
    }
...
function renderSuggestions(list){
  ...
  list.forEach(p=>{
    ...
    div.innerHTML=`<span>${p.producto||"-"}</span><span class="s-meta">${money(p.precio_venta||0)} · Stock ${Number(p.stock||0).toLocaleString("es-CO")}</span>`;
